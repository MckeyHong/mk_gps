/*global jQuery, func_url, aColumn, aError, title_add, title_edit, error_del, del_msg, del_msg_multi, now_url, default_city, return_msg_multi, error_return*/
/*property _method, _redirect, _token, ajax_response_process, alert, callback, click, confirm, cust_ajax, data, del, each, find, html, id, length, message, modal, parent, push, ready, remove, serialize, show_loading, submit, success, text, trim, url, val */
...
jQuery(document).ready(function ($) {
    ...
    var updateInfo = function () {
        ...
        oTr.find('.city_name').html($('#data-form #city :selected').text());
    ...
    /* 整批歸還 */
    $('#btn-all-return').click(function () {
        var aID = [];
        $('#list-table tbody tr td:first-child input:checkbox:checked[name=chkDel]').each(function () {
            aID.push($(this).data('id'));
        });

        if (aID.length < 1) {
            $.alert({message: error_return});
            return false;
        }

        $.confirm({
            message: return_msg_multi,
            callback: function () {
                $.show_loading();
                $.cust_ajax({
                    url: func_url + '/ReturnMulti',
                    data: {_token: $('#data-form input[name=_token]').val(), _redirect: now_url, id: aID}
                });
            }
        });
